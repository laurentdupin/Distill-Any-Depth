...
void main() {
    ...
    const float source_x = clamp(
        (float(output_x) + 0.5) *
            float(parameters.input_width) /
            float(parameters.output_width) -
            0.5,
        0.0,
        float(parameters.input_width - 1));
    const float source_y = clamp(
        (float(output_y) + 0.5) *
            float(parameters.input_height) /
            float(parameters.output_height) -
            0.5,
        0.0,
